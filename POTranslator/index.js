...
Guidelines: The GPT should be sensitive to the context of multiplayer travel game and use language appropriate for the nowdays era in game translations. For short phrase translations, it should ensure accuracy and context-appropriate language in the specified target language. It should handle game-specific terms with accuracy, ensuring that translations fit the game's narrative and style.

Personalization: The GPT will maintain a professional tone, focusing on delivering clear and precise translations without engaging in additional dialogue or seeking clarifications. If there are multiple options to translate pick only one. If translation is the same in the target language leave it as it is without additional comments.
    `;
...
const run = async (targetLanguage) => {
    const origin = fs.readFileSync('./data/data.po', 'utf-8');
    const parts = origin.split('\n\n');
    const resultParts = [];
    let i = 0;
    for (let part of parts) {
        let newParts = part.split("\n");
        let msgid = newParts.find(n => n.includes('msgid'))?.replace(`msgid `, '')?.replace(/^"/, '')?.replace(/"$/, '');
        let msgstr = (await translate(getChatModel(), msgid, targetLanguage)).content?.replace(/^"/, '')?.replace(/"$/, '');
        resultParts.push(part.replace('msgstr ""', `msgstr "${msgstr}"`));
        console.log(`${i}/${parts.length}`, msgid, ' ---> ', msgstr)
        i++;
    }
    return resultParts;
};
...
run(lang).then(result => {
    fs.writeFileSync(`./data/translations/${lang}.po`, result.join("\n\n"), 'utf-8');
});
